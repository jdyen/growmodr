...
plot.grow_mod <- function(x, ...) {
  ...
  plot_data <- array(NA, dim = c(data_tmp$n_plot, 4, data_tmp$n_block))
  for (i in 1:data_tmp$n_block) {
    row_id <- grep(paste0('plot\\[[[:digit:]]*,', i, '\\]'), rownames(mod_tmp))
    plot_data[, , i] <- mod_tmp[row_id, c('mean', '2.5%', '50%', '97.5%')]
    x_plot <- data_tmp$age_plot
    y_lims <- c(min(c(plot_data[, , i], data_tmp$size_data[which(data_tmp$block_data == i)])),
                max(c(plot_data[, , i], data_tmp$size_data[which(data_tmp$block_data == i)])))
    plot(plot_data[, 1, i] ~ x_plot,
         type = 'l',
         las = 1,
         bty = 'l',
         xlab = 'Time',
    ...
    polygon(c(x_plot, rev(x_plot)),
            c(plot_data[, 2, i], rev(plot_data[, 4, i])),
            col = 'grey65',
            border = NA)
    lines(plot_data[, 1, i] ~ x_plot, lty = 1, lwd = 1.2)
    lines(plot_data[, 3, i] ~ x_plot, lty = 2, lwd = 1.2)
