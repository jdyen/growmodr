...
gen_mod_file <- function(model, spline_params = NULL, mod_file = NULL) {
  if (model != 'spline') {
    ...
    cat(
      ...
      vector[n] age;
      int<lower=0, upper=n_block> block[n];
      int<lower=0> n_plot;
      ...
      for (i in 1:n)
      log_lik[i] = normal_lpdf(log(size[i]) | mu[i], sigma_obs);
      }',
  file = ifelse(is.null(mod_file), mod_file <- tempfile(), mod_file))
    } else {
