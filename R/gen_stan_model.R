...
# generate cross-val stan model file
gen_mod_file_cv <- function(model,
  ...
  if (!include_block) {
    if (model != 'spline') {
      ...
      num_param <- mod_par$num_par
      mod_mu <- mod_par$mu
      mu_holdout <- mod_par$mu_plot
      h_var <- NULL
      h_prior <- NULL
      h_var_holdout <- NULL
      h_holdout_prior <- NULL
      for (i in 1:num_param) {
        h_var <- paste0(h_var, 'real<lower=0> h', i, ';\n  ')
        h_var_holdout <- paste0(h_var_holdout, 'real<lower=0> h', i, '_holdout;\n  ')
        h_prior <- paste0(h_prior, 'h', i, ' ~ normal(0.0, 2.0);\n  ')
        h_holdout_prior <- paste0(h_holdout_prior, 'h', i, '_holdout = h', i, ';\n  ')
      }
      cat(
        'data {
        int<lower=0> n;
        ...
        vector<lower=0>[n_holdout] size_data_holdout;
        vector[n_holdout] age_holdout;
    }
        ...
        vector<lower=0>[n_holdout] size_holdout;
        vector[n_holdout] log_lik_holdout;
        
        for (i in 1:n_holdout)
        mu_holdout[i] = ', mu_holdout, ';\n',
        ' for (i in 1:n_holdout)
        size_plot[i] = exp(mu_holdout[i]);
        for (i in 1:n_holdout)
        log_lik_holdout[i] = normal_lpdf(log(size_data_holdout[i]) | mu_holdout[i], sigma_obs);
        }
      ...
      h_holdout_prior <- NULL
      mu_var <- 'mu[i] = b_spline[age_index[i]][1] * h1'
      mu_var_holdout <- 'mu_holdout[i] = b_spline_holdout[age_index_holdout[i]][1] * h1_holdout'
      for (i in 1:num_param) {
        if (i > 1) {
          mu_var <- paste0(mu_var, ' + b_spline[age_index[i]][', i, '] * h', i)
          mu_var_holdout <- paste0(mu_var_holdout, ' + b_spline_holdout[age_index_holdout[i]][', i, '] * h', i, '_holdout')
        }
        h_var <- paste0(h_var, 'real h', i, ';\n  ')
        h_prior <- paste0(h_prior, 'h', i, '~ normal(0.0, 2.0);\n  ')
        h_var_holdout <- paste0(h_var, 'real h', i, '_holdout;\n  ')
        h_holdout_prior <- paste0(h_prior, 'h', i, '_holdout = h', i, ';\n  ')
      }
      mu_var <- paste0(mu_var, ';\n  ')
      mu_var_holdout <- paste0(mu_var_holdout, ';\n  ')
      cat(
        'data {
            int<lower=0> n;
            vector<lower=0>[n] size_data;
            ...
            int<lower=0> n_k;
            int<lower=0> age_index[n];
            int<lower=0> age_index_holdout[n_holdout];
            row_vector[n_k] b_spline[n_age];
            int<lower=0> n_holdout;
            vector<lower=0>[n_holdout] size_data_holdout;
            vector[n_holdout] age_holdout;
            row_vector[n_k] b_spline_holdout[n_holdout];
        }
        ...
        mu_var_holdout,
        'for (i in 1:n_holdout)
            size_holdout[i] = exp(mu_var_holdout[i]);
            for (i in 1:n_holdout)
            log_lik_holdout[i] = normal_lpdf(log(size_data_holdout[i]) | mu_var_holdout[i], sigma_obs);
            }
    ...
    if (include_pred) {
      if (model != 'spline') {
        ...
        h_prior <- NULL
        sdh_prior <- NULL
        psi_var_holdout <- NULL
        psi_reg_var_holdout <- NULL
        h_holdout <- NULL
        h_prior_holdout <- NULL
        x_holdout <- NULL
        for (i in 1:num_param) {
          h_var <- paste0(h_var, 'vector<lower=0>[n_block] h', i, ';\n  ')
          h_holdout <- paste0(h_holdout, 'vector<lower=0>[n_block_holdout] h', i, '_holdout;\n  ')
          nxh_var <- paste0(nxh_var, 'int<lower=0> n_x', i, ';\n  ')
          sdh_var <- paste0(sdh_var, 'real<lower=0> sd_h', i, ';\n  ')
          x_var <- paste0(x_var, 'matrix[n_block, n_x', i, '] x', i, ';\n  ')
          x_holdout <- paste0(x_holdout, 'matrix[n_block_holdout, n_x', i, '] x', i, '_holdout;\n  ')
          b_var <- paste0(b_var, 'vector[n_x', i, '] b', i, ';\n  ')
          psi_var <- paste0(psi_var, 'vector[n_block] psi', i, ';\n  ')
          psi_reg_var <- paste0(psi_reg_var, 'psi', i, ' = x', i, ' * b', i, ';\n  ')
          h_prior <- paste0(h_prior, 'h', i, ' ~ normal(psi', i, ', sd_h', i, ');\n  ')
          b_prior <- paste0(b_prior, 'b', i, ' ~ normal(0.0, 10.0);\n  ')
          sdh_prior <- paste0(sdh_prior, 'sd_h', i, ' ~ normal(0.0, 2.0);\n  ')
          psi_var_holdout <- paste0(psi_var_holdout, 'vector[n_block_holdout] psi_holdout', i, ';\n  ')
          psi_reg_var_holdout <- paste0(psi_reg_var_holdout, 'psi_holdout', i, ' = x', i, '_holdout * b', i, ';\n  ')
          h_prior_holdout <- paste0(h_prior_holdout, 'h', i, '_holdout ~ normal(psi_holdout', i, ', sd_h', i, ');\n  ')
        }
        cat(
          'data {
                    int<lower=0> n;
          ...
          'vector<lower=0>[n] size_data;
                    ...
                    vector[n_holdout] age_holdout;
                    int<lower=0, upper=n_block> block_data[n];
                    int<lower=0, upper=n_block_holdout> block_data_holdout[n_holdout];
                }
          ...
          '}
                    ...
                    generated quantities {
                    matrix[n_holdout, n_block_holdout] mu_holdout;
                    matrix<lower=0>[n_holdout, n_block_holdout] size_holdout;
                    vector[n_holdout] log_lik_holdout;
                    
                    for (i in 1:n_holdout)
                    for (j in 1:n_block_holdout)
                    mu_plot_growth[i, j] = ', mu_holdout, ';\n',
          ' for (i in 1:n_holdout)
                    for (j in 1:n_block_holdout)
                    size_holdout[i, j] = exp(mu_holdout[i, j]);
                    for (i in 1:n_holdout)
                    log_lik[i] = normal_lpdf(log(size_data_holdout[i]) | mu_holdout[i], sigma_obs);
                    }
